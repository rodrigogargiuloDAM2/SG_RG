...
public class ModPerfil extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Cursor datosPerfil=db.rawQuery("SELECT  nombre, apellido, email, telefono, contrasegna FROM usuarios where email='"+DatoEmailM+"'",null);
        while (datosPerfil.moveToNext()) {

            //txtID.setText(datosPerfil.getString(0));
            txtNombre.setText( datosPerfil.getString(0));
            txtApellido.setText( datosPerfil.getString(1));
            txtEmail.setText(datosPerfil.getString(2));
            txtTelefono.setText(datosPerfil.getString(3));
            txtContrasegna.setText(datosPerfil.getString(4));
        }
     ...
     SQLiteDatabase db = conn.getWritableDatabase(); //abrimos BBDD
        ...
        if (!nombre.isEmpty() && !apellido.isEmpty() && !email.isEmpty() && !telefono.isEmpty() && !contrasegna.isEmpty()){
            ...
            if (existe==false){
                ...
                int cantidad = db.update("usuarios",registro,"email='"+ DatoEmailM+"'",null);
                db.close();
    ...
    public void EliminarCuenta(View view){
       ...
       // String id=txtID.getText().toString();
        ...
        int cantidad = db.delete("usuarios","email='" + DatoEmailM+"'", null);  //cantidad de articulos borrados ,, metodo delete


        int productos= db.delete("productos","id_email='"+ txtEmail.getText()+"'",null);
        db.close();

        if (productos != 0){
            Toast.makeText(this, "Artículos eliminados.", Toast.LENGTH_SHORT).show();
        }else {
            Toast.makeText(this, "Error?/ O ningún artículo a eliminar", Toast.LENGTH_SHORT).show();
        }
